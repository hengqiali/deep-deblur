...
class GoProDataset(Dataset):
    def __init__(self, blur_image_files, sharp_image_files, root_dir, crop=False, crop_size=256, multi_scale=False, rotation=False, color_augment=False, gaussianNoise = False, verticalFlip = False, transform=None):
        """
        Args:
    ...
    def __getitem__(self, idx):
        ...
        #blur_image = readgen(os.path.join(self.root_dir, image_name[0], image_name[1], image_name[2], image_name[3])).astype(np.float32)
        #sharp_image = readgen(os.path.join(self.root_dir, image_name[0], image_name[1], 'sharp', image_name[3])).astype(np.float32)
        ...
        if self.gaussianNoise:
            blur_image = blur_image.numpy()
            sharp_image = sharp_image.numpy()
            shape = blur_image.shape
            gaussian_noise = np.random.normal(0, 1e-4, shape)    
            blur_image = (blur_image + gaussian_noise).clip(0,1)
            sharp_image = (sharp_image + gaussian_noise).clip(0,1)
            blur_image = (torch.from_numpy(blur_image)).float()
            sharp_image = (torch.from_numpy(sharp_image)).float()
        ...
        if self.verticalFlip:
            if random.random() < 0.5:
                ...
                blur_image = np.copy(np.flipud(blur_image))
                sharp_image = np.copy(np.flipud(sharp_image))
                blur_image = (torch.from_numpy(blur_image)).float()
                sharp_image = (torch.from_numpy(sharp_image)).float()
        """
        if self.multi_scale:
