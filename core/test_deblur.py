...
def test_deblurnet(cfg, epoch_idx, test_data_loader, deblurnet, test_writer):
    ...
    for batch_idx, (imgs, labels) in enumerate(test_data_loader):
        ...
        with torch.no_grad():
            ...
            #img_blur_left = images['blur_image'].cuda()
            #img_clear_left = images['sharp_image'].cuda()
            #print('test ' + str(img_blur_left.shape))
            # Test the decoder
            output_img_clear_left = deblurnet(img_blur_left)

            # Append loss and accuracy to average metrics
            img_PSNR = PSNR(output_img_clear_left, img_clear_left)
            ...
            if batch_idx < cfg.TEST.VISUALIZATION_NUM:
                if epoch_idx == 0 or cfg.NETWORK.PHASE in ['test', 'resume']:
                    test_writer.add_image('DeblurNet/IMG_BLUR_LEFT'+str(batch_idx+1),
                                          imgs[0][[2,1,0],:,:] + torch.Tensor(cfg.DATA.MEAN).view(3, 1, 1), epoch_idx+1)
                    test_writer.add_image('DeblurNet/IMG_CLEAR_LEFT' + str(batch_idx + 1),
                                          labels[0][[2,1,0],:,:] + torch.Tensor(cfg.DATA.MEAN).view(3, 1, 1), epoch_idx + 1)
            ...
            if cfg.NETWORK.PHASE == 'test':
                left_out_dir = os.path.join('output/images','single',name,'left')
    ...
    if cfg.NETWORK.PHASE == 'test':
        ...
        result_file = open(os.path.join('/data/zhuyu/hq/DAVApkernel/output/', 'test_result.txt'), 'w')
